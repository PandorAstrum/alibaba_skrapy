...
class entire_app:
	def __init__(self, top=None):
		...
		self.pre_selected_selected_label.place(relx=0.56, rely=0.697, height=31, width=261, bordermode='ignore')
		...
		self.status_frame.place(relx=0.0, rely=0.861, relheight=0.119, relwidth=0.989)
		...
		self.remaining_label.place(relx=0.016, rely=0.349, height=21, width=614, bordermode='ignore')
		...
		self.remaining_label.configure(text='''Remaining 512 out of 600''')
		self.remaining_label.configure(width=614)

		self.image_dl_frame = tk.LabelFrame(top)
		self.image_dl_frame.place(relx=0.521, rely=0.472, relheight=0.403, relwidth=0.47)
		...
		self.img_csv_selected_label.configure(text='''scrapped_5_11_2019.csv''')
	...
	def browsecsvfile(self):
		csvfilename = filedialog.askopenfilename(title="Select A File", filetype=(("csv", "*.csv"), ("All Files", "*.*")))
		self._previous_csv = csvfilename
		# update label
		_fileName = csvfilename.rsplit("/", -1)[-1]
		self.pre_selected_selected_label.configure(text=_fileName)

	def browsedir(self):
		self._directory = filedialog.askdirectory(title="Select A Directory")
		# change the directory path to this
		self.out_folder_selected_label.configure(text=self._directory)
	...
	def downloaddata(self):
		...
		# take settings
		self._category_check = ui_support.che57.get()
		self._directory = self.out_folder_selected_label.cget("text")
		self._filename = self.output_folder_entry.get()
		# prepare file name _directory + _filename
		self._user_agents = self.user_agents_entry.get()

		# start scraping
		# update stats periodically
		self.remaining_label.configure(text=self.user_agents_entry.get())
		# disable all
